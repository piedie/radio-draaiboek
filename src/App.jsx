...
const RadioRundownPro = () => {
  ...
  const loadUserData = async (userId) => {
    try {
      ...
      const { data: membershipData, error: membershipError } = await supabase
        .from('program_memberships')
        // Belangrijk: altijd filteren op user_id, anders kan dit (heel) traag worden onder RLS.
      ...
      if (membershipError) {
        ...
        // Laad de programs in een aparte query (sneller en minder complex dan een nested select).
        const programIds = Array.from(new Set(normalized.map((m) => m.program_id).filter(Boolean)));
        let programsById = {};
        if (programIds.length > 0) {
          ...
          if (programsError) {
            ...
            programsById = Object.fromEntries((programsData || []).map((p) => [p.id, p]));
          }
        }

        const normalizedWithProgram = normalized.map((m) => ({
          ...m,
          program: programsById[m.program_id] || null,
        }));

        setMemberships(normalizedWithProgram);
        const uniquePrograms = Array.from(
          new Map(
            normalizedWithProgram
              .filter((m) => m.program)
        ...
        const first = normalizedWithProgram.find((m) => m.program_id);
        if (first?.program_id) {
  ...
  // Crash/debug zichtbaar maken in productie (alleen console + kleine banner)
  const [fatalError, setFatalError] = useState(null);

  useEffect(() => {
    const onError = (eventOrMessage, source, lineno, colno, error) => {
      const message = typeof eventOrMessage === 'string' ? eventOrMessage : eventOrMessage?.message;
      const stack = error?.stack || eventOrMessage?.error?.stack;

      // Log altijd volledig, zodat we de eerste echte crash zien (ook bij minified bundles)
      console.error('[FATAL window.onerror]', { message, source, lineno, colno, error, stack });

      // Zet 1x een UI-banner (niet spammen)
      setFatalError((prev) => prev ?? { type: 'error', message: message || 'Onbekende fout', stack, source, lineno, colno });
    };
    ...
    const onUnhandledRejection = (event) => {
      ...
      console.error('[FATAL unhandledrejection]', { message, reason, stack });
      setFatalError((prev) => prev ?? { type: 'unhandledrejection', message, stack });
    };
  ...
  return (
    <div className={`${t.bg} min-h-screen p-3`}>
      {/* Debug banner: toont de eerste fatale error. (Niet verwijderen tot issue opgelost is.) */}
      {fatalError && (
        <div className="max-w-[2200px] mx-auto px-2 mb-3">
          <div className="rounded border border-red-500 bg-red-50 text-red-800 px-4 py-3 text-sm">
            <div className="font-semibold">App error: {fatalError.message}</div>
            <div className="text-xs mt-1 opacity-80">Open DevTools Console voor de volledige stacktrace.</div>
          </div>
        </div>
      )}
          ...
          <div className={`grid gap-6 ${showClock ? 'grid-cols-1 lg:grid-cols-2' : 'grid-cols-1'}`}>
            ...
            {showClock && (
              <div className="space-y-6">
                ...
                {showJingleEditor && (
                  <div className={`${t.card} rounded-lg p-6 shadow border ${t.border}`}>
                    <h3 className={`text-lg font-bold mb-4 ${t.text}`}>🔔 Jingles</h3>
      ...
      {showAdminPanel && (
        ...
        >
          <AdminPanel
            ...
            onRoleChange={(nextRole) => setMyRole(nextRole)}
            onClose={() => {
